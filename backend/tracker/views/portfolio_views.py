...
def manage_api_keys(request):
    ...
    - POST Request: Will save or update API keys for Binance and Bybit.
    - GET Request: Retrieve saved API keys with masking to prevent hackers from copying.
    """
    
    if request.method == 'POST':
        try:
            # Retrieve or create an APIKey entry for the user
            api_keys, created = APIKey.objects.get_or_create(user=request.user)
            
            # Update only if keys are provided in the request
            ...
            # Verify Binance API keys by making a test request
            try:
                if api_keys.binance_api_key and api_keys.binance_secret_key:
                    timestamp = str(int(time.time() * 1000))
